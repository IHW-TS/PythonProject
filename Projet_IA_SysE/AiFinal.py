...
def solve(vec, moves):
    ...
    if vec[2] == 0: 
        # La fonction valid_add retourne une liste d'actions qui sont valides pour être effectuées à partir de l'état actuel représenté par le vecteur ( quand le bateau est égale à 0)
        actions = valid_add(vec)
    else:
        # Idem avec valid_substract
        actions = valid_substract(vec)

    for action in actions:
        # Créer une nouvelle copie du vecteur d'état actuel
        # new_vector est une copie indépendante de vec, ce qui permet de changer la liste sans affecter l'autre.
        new_vector = copy.deepcopy(vec)
        for i in range(3):
            # Créer une nouvelle copie du vecteur d'état actuel
            # Si new_vector[2] est égal à 0, l'action est ajoutée à l'élément correspondant du vecteur, sinon elle est soustraite. Cela modifie le vecteur d'état pour refléter les actions prises pour résoudre le problème des missionnaires et des cannibales.
            if new_vector[2] == 0:
...
# Appelle la fonction `solve` pour trouver une solution pour le vecteur `start` et initialiser à 0 mouvements.
solve(start, 0)
